...
Run:
  ...
  python3 -u ~/aksarallm_formal_benchmark.py 2>&1 | tee ~/benchmark.log
"""
...
def main():
    ...
    # Save
    report_path = os.path.expanduser("~/benchmark_report.md")
    with open(report_path, "w") as f:
        f.write(report_text)
    
    json_path = os.path.expanduser("~/benchmark_results.json")
    results_data = {
    ...
    try:
        ...
        api.upload_file(path_or_fileobj=report_path, path_in_repo=f"benchmark_{ts}.md", repo_id=EVAL_REPO, token=HF_TOKEN)
        api.upload_file(path_or_fileobj=json_path, path_in_repo=f"benchmark_{ts}.json", repo_id=EVAL_REPO, token=HF_TOKEN)
    ...
    # Upload model card
    try:
        model_card_path = os.path.expanduser("~/aksarallm_model_card.md")
